...
function findTour(band) {
    return db.query(`SELECT tours.id, tour_date, venues.name, venues.city, venues.state, venues.id AS venue_id FROM tours, venues WHERE tours.band_id=$1 AND tours.venue_id = venues.id`, [band.id]);
}
...
module.exports = { findTour, create, findDate, update };
